...
$("#sendMic").on("click", function(e){
    const searchMic = document.querySelector("#sendMic");
    ...
    recognition.onresult = function(event){
        ...
        const regex = /(\d)\s+(?=\d)/g;
        const subst = `$1`;
        const result = transcript.replace(regex, subst);
...
$('.chat_header_title').click(function(){
    $(".profile_div").toggle("slow");
    $(".widget").toggle("slow");
    scrollToBottomOfResults();
})
